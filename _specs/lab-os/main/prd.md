...
- The adherence gates (`docs-budget`, `merge-bar-check`) are callable by any
...
- Staging of rule edits. Caravan is the staging surface — D17, recorded in Caravan's own
  main-bundle decisions document (Caravan #2); this repo receives the round-trip.

## Constraints

- **Byte budgets** bind the always-loaded tier. Which surfaces are
...
- **Every standing decision lives in `spec.md`**, this bundle's decisions document, as a
  numbered section — there is no separate project-altitude surface any more.
...
- **Is lab-os a workspace root, or a member repo whose main bundle belongs at `_specs/main/`?**
  The bundle-path convention reserves the `<repo>` path segment for workspace-root altitude,
  while this bundle records Caravan as the shared dev home — which would read lab-os as a
  member repo. Decided for now, and stated once in [spec.md](./spec.md) §R1. *Trigger:* a
  ruling that settles lab-os's altitude, or lab-os ceasing to be worked as its own root.
