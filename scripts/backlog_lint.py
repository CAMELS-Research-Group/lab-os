...
vanished):
  ...
  - public-tier spot check over EVERY line, not just Items+Inbox: Item
    titles + full field values (including continuation lines and unattached
    text), the Inbox section, AND every otherwise-unowned non-blank line —
    the preamble before `## Index`, the `## Index` region, and any text under
    a stray heading — all captured as orphans and leak-scanned, so a private
    reference anywhere in the file is caught, not only inside Items/Inbox.
...
def dep_value_error(raw: str) -> str | None:
    """Validate a `Depends on` value against the dependency grammar.

    Returns None when `raw` is well-formed — a no-deps sentinel
    ("—" / "-" / "" / "none") or a list of one-or-more `B<number>` ids
    (comma/space separated) and nothing else — and a one-line reason string
    otherwise. This is the SINGLE validity check for the grammar, shared by
    lint() and the follow-on view/digest source-integrity guard (PR #68) so a
    value parse_deps silently reads as [] (prose like "blocked on Watson", a
    lowercase `b2`, a stray `TBD`, or pure punctuation) fails closed in EVERY
    tool — never letting a genuinely-blocked item advertise as "unblocked".
    parse_deps stays permissive (it just extracts ids); callers gate on this.
    """
    if raw.strip() in _DEP_NONE:
        return None
    ids = re.findall(r"B\d+", raw)
    leftover = re.sub(r"B\d+", "", raw).strip(" \t,;")
    if leftover or not ids:
        return (f"'Depends on' value {raw.strip()!r} is neither a no-deps "
                "sentinel (— / none) nor `B<n>` ids")
    return None
    ...
    for i, line in enumerate(lines, start=1):
        ...
        if section != "## Items":
            ...
            if _ITEM_HEAD.match(line):
                parse_errors.append(
                    f"item heading outside Items section at line {i}: {stripped}")
            # Capture EVERY otherwise-unowned non-blank line — the preamble
            # before `## Index`, non-table and table lines in the `## Index`
            # region, and anything under a stray `## <heading>` — as an orphan
            # so the leak scan sees it. Without this, a private/gated reference
            # outside Items+Inbox (header prose, an Index cell, a stray-heading
            # region) escaped every scan and `--check --enforce` went green on
            # a real leak. This adds no parse error and no false positive on a
            # clean file: orphans only surface when a line matches a private
            # pattern.
            if stripped:
                orphans.append((i, line))
            continue
...
def lint(backlog: Backlog, req_fields: list[str]) -> Findings:
    ...
    # Orphans are every otherwise-unowned non-blank line: unattached
    # Items-section text (also a hard parse error above), the preamble, the
    # `## Index` region, and any stray-heading region. Scan them all for leaks,
    # so a non-public reference anywhere outside the Items/Inbox surfaces above
    # is named rather than escaping every check.
    for lineno, text in backlog.orphans:
        for pat in _PRIVATE_PATTERNS:
            if pat.search(text):
                f.errors.append(f"line {lineno}: unowned text embeds a "
    ...
    for it in backlog.items:
        ...
        # every tool that reuses the grammar. dep_value_error is the one
        # validity check, shared with the #68 view/digest integrity guard.
        dep_err = dep_value_error(raw_dep)
        if dep_err:
            f.errors.append(f"{it.id}: {dep_err}")
        for r in refs:
...
def _harden_stdout() -> None:
    """Make output tolerate non-ASCII under ANY locale, so no print can crash.

    The findings, argparse help, and the fail-closed one-liner all embed
    literal em-dashes. Under `LC_ALL=C` / `PYTHONUTF8=0` stdout defaults to
    the ASCII codec, so a single em-dash raises UnicodeEncodeError mid-run —
    inverting the warn-only posture into a false-red traceback and aborting
    the self-test partway. Re-encode stdout/stderr as UTF-8 with `replace`
    at startup so every output path (including the fail-closed print of its
    own message) is safe. Guarded: a redirected/older stream without
    reconfigure() (the self-test's StringIO, which handles Unicode natively)
    is left untouched."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.self_test:
        # The self-test fixtures + assertions live in a sibling module so this
        # production file stays under the Class-1 1,000-line budget
        # (reference/code-quality-taxonomy.md). The `--self-test` flag remains
        # the single entry point; the CLI contract is unchanged.
        from backlog_lint_selftest import run_self_test
