...
- `.claude/skills/` + `.claude/commands/` — shared Claude Code skills and their slash commands. Currently: [`pr-round`](.claude/skills/pr-round/SKILL.md) (`/pr-round`) — one round of PR work across every PR connected to you: review others' PRs, remediate review feedback on your own. Note: the tier-2 rubric layer and the specialist panel silently absent themselves until #58 lands — a bare `lab-os` clone gets tier 1 + universal rubric only (details: SKILL.md § Deployment). Provenance: [`ATTRIBUTION.md`](.claude/skills/ATTRIBUTION.md).
