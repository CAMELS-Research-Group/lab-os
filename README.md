...
- `.claude/rules/` — markdown files defining lab-wide conventions. Consumed by Cowork locally (natively in a lab-os fork, or via a junction/symlink in the multi-repo setup) and by the PR-review GitHub Action at review time.
  ...
  - [`06-timeboxing.md`](.claude/rules/06-timeboxing.md) — timeboxing standard (session boxes, agent task boxes, calibration loop; owning docs under `docs/`). The `05` slot is reserved for the agent-runtime rule in flight as #58, not a gap.
