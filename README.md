...
- `.claude/plugins/` — Claude Code plugins the lab owns, installed through the marketplace below rather than copied per machine. Currently: [`context-gc`](.claude/plugins/context-gc/README.md) — re-injects session state (files in flight, task list, and a fail-open local-Ollama read on objective/decisions) after the harness auto-compacts, via a `SessionStart(compact)` hook. Unlike rules, agents and skills, a plugin ships executable code that runs on every installer's machine — see the security boundary noted above.
- [`.claude-plugin/marketplace.json`](.claude-plugin/marketplace.json) — the org-owned plugin marketplace manifest (`lab-os`), the distribution point for everything under `.claude/plugins/`. Add it once with `/plugin marketplace add CAMELS-Research-Group/lab-os`, then `/plugin install <name>@lab-os`.
