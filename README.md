...
- `.claude/skills/` + `.claude/commands/` — shared Claude Code skills and their slash commands. Currently: [`pr-round`](.claude/skills/pr-round/SKILL.md) (`/pr-round`) — one round of PR work across every PR connected to you: review others' PRs, remediate review feedback on your own. Note: from a bare `lab-os` clone the tier-2 rubric layer is absent until the rules-parity sync (#58) lands, and the specialist panel runs only when the dev home carries `.claude/agents/` + `specialist-dispatch.md`; both degrade silently (details: SKILL.md § Deployment and § The specialist panel). Provenance: [`ATTRIBUTION.md`](.claude/skills/ATTRIBUTION.md).
