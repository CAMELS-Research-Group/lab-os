...
- **Target file: the repo where the task lives.** Rows are appended to the
  calibration file of the repo the ticket/task belongs to —
  `docs/timebox_calibration.mdx` for lab-os work, else that repo's
  `timebox_calibration.md`, created on first row with the standard header.
  The reference class lives beside the work it measures; a cross-repo
  roll-up is a grep away. `TIMEBOX_CALIBRATION_FILE` env var overrides when
  set.
