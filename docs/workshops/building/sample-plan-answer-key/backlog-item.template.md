<!--
Backlog item template. Copy one block per item into BACKLOG.md under ## Items.
Keep each field to a line or two — a backlog item is a pointer to work, not the plan itself.
The "Done when" line is load-bearing: if you can't state a single checkable condition, the item
is too big or too vague to be ready. Delete this comment when you copy the block.
Teaching copy. The live template is `templates/backlog-item.template.md` — edit that one.
-->

## <id> — <one-line title>

- **Problem:** <what is broken or missing, and for whom>
- **Who it helps:** <the person or workflow that benefits>
- **Value:** <why it is worth doing now rather than later>
- **Rough size:** <S = one sitting · M = a few · L = a project — split anything L before it is ready>
- **Done when:** <the single observable check that proves it shipped — a command, a behaviour, a file>
- **Depends on:** <other item ids, or —>
- **Status:** <inbox = raw idea · ready = shaped and unblocked · in-progress · done>
