...
- 2026-06-10 16:34 · task 6 · gate: Supersedes + timestamp greps hit; parser anchors: fixed `## Standing Decisions`/`## Entries` H2s, entry headers only other `##` allowed; constraint for task 9: lint regexes must be digit-anchored (template comments carry `YYYY-MM-DD`-token grammar examples that must not match)
- 2026-06-10 16:35 · task 7 · gate: 3,321 B ≤ 4,096; spec review fix: budget line gained warn-only-until-first-green qualifier, dropped inaccurate "hard cap" phrasing
- 2026-06-10 16:35 · task 8 · gate: all three files exist, Execution Log grep hits; spec review fix: design template status line moved directly under H1 (was below the lifecycle comment)
- 2026-06-10 17:25 · task 9 · gate: `python scripts/log_lint.py --self-test` 20 cases pass, exit 0, unpiped; code review fixes: multiset self-test assertions, utf-8-sig BOM tolerance, workflow runs branch-local script when caller is lab-rules (repo-name-gated)
- 2026-06-10 17:25 · task 10 · gate: `python scripts/docs_budget.py --self-test` all checks pass, exit 0, unpiped; code review fix: unguarded stat() wrapped (unreadable surfaces warn+skip, never fail), same lab-rules script-resolution fix
- 2026-06-10 17:25 · task 11 · gate: `python scripts/merge_bar_check.py --self-test` 9/9 pass, exit 0, unpiped; code review fixes: HTML-comment spans stripped (commented checkbox/heading no longer satisfies rules), job-level pull_request guard, same script-resolution fix, fallback-drift guard
- 2026-06-10 17:25 · tasks 9–11 · shared altitude call: all three workflows resolve the script from the caller checkout when `$GITHUB_REPOSITORY` is WatsonWBlair/lab-rules (else lab-rules@main checkout) — self-caller PRs exercise the branch copy; repo-name gate prevents downstream shadowing
- 2026-06-10 17:40 · task 12 · gate: YAML parses, uses: paths + with: inputs match declared workflows (static — live `gh run` re-verified after PR opens per plan caveat); spec review fix folded in: docs-budget.yml gained the `permissions: contents: read` block the other two workflows already declared
- 2026-06-10 17:55 · task 13 · deviation: plan said Modify, but project_log.md exists only on the unmerged local branch docs/onboarding-project — file CREATED fresh from the template, preserving that branch's 2026-06-09 entry verbatim (header reformatted to 00:00) per plan acceptance; gate: log_lint exits 1 with exactly the sanctioned entry-over-budget violation on the preserved entry (1,846 B, untrimmed) — routed through the PR's log-lint:override reason; #PR-TBD ref filled at PR-open time
- 2026-06-10 17:55 · task 14 · gate: diff confined to §5 + §8, all retained bullets absent from deletion side; note: §5 had no pre-existing bypass bullet (bypass was unwritten) — pointer line ADDED rather than converted
- 2026-06-10 17:55 · task 15 · gate: PR-LIFECYCLE + standards.yml greps hit; spec review verified every added README link target exists; scope-discipline section untouched
- 2026-06-10 18:30 · task 12 (deferred gate) · PR #6 opened; standards.yml run 27307369961 completed success — docs-budget, log-lint, merge-bar-check all green; log-lint job log confirms "override label honored; lint enforcement skipped" (label + body reason both present, deliberate path); adoption entry #PR-TBD refs filled with #6 (entry unmerged, immutability not yet in force)
- 2026-06-10 18:50 · §8.3 review remediation · audit pass found a misleading diagnostic (trailing whitespace in an entry header desyncs the index↔entry key and blames the index line) — subjects now stripped at capture in log_lint.py; self-test 20/20, repro lints clean; PR body gained one sentence documenting the deliberate no-index-line call for the migrated entry
