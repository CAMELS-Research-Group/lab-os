...
**Status:** approved 2026-07-31 (Arya). Decision history: `project_log.md` (lab altitude).
**Deliverables:** `docs/timebox_recording.mdx` (guide) · `.claude/skills/timeboxing/SKILL.md` (skill).

## Problem

The lab timeboxing standard (`docs/timeboxing.mdx`, v1.0) defines a calibration loop — state box + exit criterion at start, append a planned-vs-actual row at box end — but it depends entirely on manual discipline. `docs/timebox_calibration.mdx` has zero rows since adoption (2026-07-24): the loop has never closed.
